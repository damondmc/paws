...
class SearchParamGenerator:    
    ...
    def __init__(self, fBand=0.1, freqDerivOrder=2, nc_min=2, nc_max=7):
        ...
        Parameters:
        - fBand (float): Frequency band width for each search segment (default: 0.1 Hz).
        - freqDerivOrder (int): Order of frequency derivatives to consider (default: 2 for f1dot and f2dot).
    ...
    def genParamTable(self, freq, nf1dots, nf2dots):
        ...
        - nf1dots (int): Number of segments for f1dot range.
        - nf2dots (int): Number of segments for f2dot range.
        ...
        for i in range(int(1.0/self.fBand)):
            ...
            f0min, _, f0band = fr.f0BroadRange(f0, self.fBand)
            for j in range(nf1dots):
                ...
                f1max = f1min + f1Band
                if j == nf1dots - 1:
                    f1max = 0.0             # manually set f1dot upper limit to 0 (numerical accuracy/error exits)
                    f1Band = 0.0 - f1min
                ...
                for k in range(nf2dots):
                    ...
                    if j == (nf1dots-1) and k == 0:
                        f2min = 0.0
                        f2Band = f2max    # manually set f2dot lower limit to 0 (numerical accuracy/error exits)
    ...
    def genParam(self, tau, alpha, dalpha, delta, ddelta, fmin, fmax, df1dot=1e-9, df2dot=1e-19):
        ...
        Parameters:
            ...
            - fmin (int): Minimum starting frequency (in Hz).
            - fmax (int): Maximum starting frequency (in Hz).
            - df1dot (float): Desired resolution for f1dot (default: 1e-9 Hz/s).
            - df2dot (float): Desired resolution for f2dot (default: 1e-19 Hz/s).
